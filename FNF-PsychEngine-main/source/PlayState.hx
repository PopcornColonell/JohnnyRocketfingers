...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'johnbg': 
				var bg:BGSprite = new BGSprite('1back', -550, -280, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('1layer1', 800, 370, 0.9, 0.9);
				stageFront.updateHitbox();
				add(stageFront);

				johncat = new BGSprite('cat_johnny', 200, 520, 1, 1, ['cat_lol']);
				add(johncat);
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['ur_bad', 'ur_good', 'hype', 'two_keys', 'toastie', 'debugger', 'suppish_nomiss', 'brutality_nomiss', 'kickass_nomiss', 'weekj_nomiss', 'infijohnny_nomiss']);
				...
				switch(achievementName)
				{
					case 'weekj_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'weekj':
									if(achievementName == 'weekj_nomiss') unlock = true;
							}
						}
					case 'ur_bad':
					...
					case 'suppish_nomiss':
						if(Paths.formatToSongPath(SONG.song) == 'suppish' && !usedPractice && songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && !changedDifficulty ) {
							unlock = true;
						}
					case 'brutality_nomiss':
						if(Paths.formatToSongPath(SONG.song) == 'brutality' && !usedPractice && songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && !changedDifficulty ) {
							unlock = true;
						}
					case 'kickass_nomiss':
						if(Paths.formatToSongPath(SONG.song) == 'kickass' && !usedPractice && songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && !changedDifficulty ) {
							unlock = true;
						}
					case 'infijohnny_nomiss':
						if(Paths.formatToSongPath(SONG.song) == 'infijohnny' && !usedPractice && songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && !changedDifficulty ) {
							unlock = true;
						}
				}
